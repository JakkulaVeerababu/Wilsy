...
export const filterKeys=['q','time','stage','mode','role','country','company','skill','type','seniority','experience','salary','currency','period','minimum','visa','relocation','sort'];
...
export function apiFilters(state,savedIds=[]) {
  const f=Object.fromEntries(filterKeys.filter(k=>state[k]).map(k=>[k,state[k]]));
  if(!f.currency||!f.period)delete f.minimum;
...
export function jobSalary(job,fx) {
  ...
  if(range.min==null&&range.max==null)return {primary:job.salary_text||'Salary not disclosed',secondary:'',note:''};
  const period=job.salary_period?` / ${label(job.salary_period).toLowerCase()}`:' · period not specified';
  ...
  if(curr==='USD')return {primary:amount(n=>inr(n,fx.rate))+period,secondary:amount(usd)+period+' · original USD',note:`INR estimate · ₹${fx.rate}/USD · ${fx.date}`};
  if(curr==='INR')return {primary:amount(n=>inr(n,1))+period,secondary:'Original INR',note:''};
